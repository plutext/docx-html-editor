 ...
 *  
*/
public abstract class SessionAwareAbstractTableWriter extends AbstractSimpleWriter {
  ...
  static {
	   ...
	   * and these can't be approximated by a background color
	   * so they are ignored
	...
	protected SessionAwareAbstractTableWriter() {
		super(WRITER_ID);
	}
...
@Override
  public Node toNode(AbstractWmlConversionContext context, Object unmarshalledNode, Node content, TransformState transformState, Document doc) throws TransformerException {
    ...
    // The tbl level customisation: store w:tblPr and w:tblGrid
    String id = getId(((TableModelTransformState)transformState).getIdx());
    tableRoot.setAttribute("id", id );
    ...
    for (int rowIndex = 0; rowIndex < table.getCells().size(); rowIndex++) {
		    ...
		    // The row level customisation: store w:trPr and w:tblPrEx
			String rowId = id+"_"+rowIndex;
			...
			for (AbstractTableWriterModelCell cell : rowModel.getRowContents()) {
				...
				if (cell.isDummy()) {
				    ...
				    // The cell level customisation: store w:tcPr 
					// - only for real cells
					String cellId = id+"_"+rowIndex+"_"+cellCounter;
